...
| Primary hooks | Tracked project-scope `.cursor/hooks.json` registers `stop`, `sessionStart`, and three `preToolUse` seatbelts, all anchored through `$CURSOR_PROJECT_DIR`. Cursor ALSO loads `<project>/.claude/settings.json`, so the tracked Claude entries stand down on a Cursor-delivered payload; `docs/turnend-guard.md` owns that predicate. |
