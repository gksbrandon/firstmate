...
Two checks run before every injection, dispatched through `bin/fm-backend.sh`
for the supervisor's own backend (tmux or herdr; see "Auto-discovered supervisor
pane" below):
...
- **Primary-pane busy guard** - `pane_is_busy` trusts Herdr native `busy` when available, otherwise matches rendered output against only the detected primary harness's signature.
  ...
  A busy pane defers unless the detected primary harness is one `fm_composer_busy_queues_input` (`bin/fm-composer-lib.sh`) has verified to queue a submitted line as its next turn, which Claude does.
  That escape exists because a firstmate primary can stay mid-turn for hours, so waiting for an idle pane means never delivering: on 2026-08-26 every escalation buffered for 71 minutes behind an absolute refusal.
  For a queueing harness the composer guard below and the verified submit are what keep the injection safe, and an unlisted harness keeps deferring rather than inheriting Claude's behavior without its own live proof.
...
A deferred injection leaves the buffered escalation in `state/.subsuper-escalations`, retried on the next housekeeping tick.
...
attempts one normal flush, which still requires an affirmatively empty composer and a pane that is either idle or on a verified queueing harness.
...
- **Max-defer escape** - the daemon must never silently wedge. If anything stays
  ...
  normal flush, which still requires an affirmatively empty composer and a pane that is either
  idle or on a verified queueing harness. If that
