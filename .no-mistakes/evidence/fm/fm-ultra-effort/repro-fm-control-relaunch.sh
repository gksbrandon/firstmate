...
dump_relaunch() { # <label> <case-dir> <id>
  printf '\n----------------------------------------------------------------\n'
  printf 'STEP: %s\n' "$1"
  printf -- '----------------------------------------------------------------\n'
  printf '  durable task metadata after relaunch:\n'
  grep -E '^(harness|model|effort)=' "$2/home/state/$3.meta" | sed 's/^/    /'
  printf '  relaunch journal (state/%s.control-relaunch):\n' "$3"
  grep -E '^(from_harness|to_harness|from_effort|to_effort|to_model)=' "$2/home/state/$3.control-relaunch" | sed 's/^/    /'
  printf '  launch command sent to the replacement pane:\n'
  grep -F 'encode launch-brief' "$2/fake/literal" | sed 's/^/    /'
}

# --- explicit --effort ultra on a codex relaunch -----------------------------
dir=$(new_case codexultra rl7b)
add_ship_task "$dir" rl7b claude
printf 'codex' > "$dir/fake/becomes"
printf '\n$ fm-control.sh rl7b relaunch --harness codex --model gpt-5.6-sol --effort ultra --note "use the ultra tier"\n'
run_control "$dir" rl7b relaunch --harness codex --model gpt-5.6-sol --effort ultra --note "use the ultra tier" | sed 's/^/  /'
dump_relaunch "explicit --effort ultra reaches the codex replacement launch" "$dir" rl7b

# --- a durable config/secondmate-harness codex+ultra pin ---------------------
dir=$(new_case smultra sm3b)
home="$dir/home"
mkdir -p "$home/config" "$home/data/sm3b"
printf 'codex gpt-5.6-sol ultra\n' > "$home/config/secondmate-harness"
...
sed 's/^/  /' "$home/config/secondmate-harness"
printf '$ fm-control.sh sm3b relaunch\n'
run_control "$dir" sm3b relaunch | sed 's/^/  /'
dump_relaunch "a durable codex+ultra secondmate pin reaches the replacement launch" "$dir" sm3b
printf '\n'
