...
. "/Users/brandon.goh/.no-mistakes/worktrees/75ea69edc5f9/01M0YQXXQYGJJAZNSM97YCNZTQ/tests/lib.sh"
# shellcheck source=tests/remote-herdr-fixture.sh
. "/Users/brandon.goh/.no-mistakes/worktrees/75ea69edc5f9/01M0YQXXQYGJJAZNSM97YCNZTQ/tests/remote-herdr-fixture.sh"

ROOT="/Users/brandon.goh/.no-mistakes/worktrees/75ea69edc5f9/01M0YQXXQYGJJAZNSM97YCNZTQ"
...
FM_EVIDENCE_STEP=0
dump_launch() { # <label>
  FM_EVIDENCE_STEP=$((FM_EVIDENCE_STEP + 1))
  printf '\n----------------------------------------------------------------\n'
  printf 'STEP %s: %s\n' "$FM_EVIDENCE_STEP" "$1"
  printf -- '----------------------------------------------------------------\n'
  printf '  config/secondmate-harness on the captain-facing primary:\n'
  sed 's/^/    /' "$PARENT/config/secondmate-harness"
  printf '  parent task metadata (%s):\n' "state/ios.meta"
  grep -E '^(harness|model|effort)=' "$PARENT/state/ios.meta" | sed 's/^/    /' || true
  if [ -f "$REMOTE_META" ]; then
    printf '  remote host task metadata:\n'
    grep -E '^(harness|model|effort)=' "$REMOTE_META" | sed 's/^/    /' || true
  fi
  printf '  launch command the remote pane received (herdr pane send-text):\n'
  grep -F 'pane send-text' "$HERDR_LOG" | sed 's/^/    /' || printf '    (none)\n'
}
...
dump_launch "codex: a configured ultra profile crosses the SSH boundary and reaches the remote pane's launch command"
...
dump_launch "pi: the shared max level still reaches the remote pi launch command"
...
dump_launch "pi: a Codex-only ultra profile is recorded on both hosts and omitted from the remote launch"
...
printf '\n----------------------------------------------------------------\n'
printf 'STEP 4: an unverified configured effort is refused before any remote launch\n'
printf -- '----------------------------------------------------------------\n'
printf '  config/secondmate-harness on the captain-facing primary:\n'
sed 's/^/    /' "$PARENT/config/secondmate-harness"
printf '  fm-spawn.sh ios --secondmate said:\n'
printf '%s\n' "$out" | grep -F 'invalid configured remote secondmate effort' | sed 's/^/    /'
printf '  launch commands the remote backend received:\n'
grep -F 'pane send-text' "$HERDR_LOG" | sed 's/^/    /' || printf '    (none - refused before any remote work)\n'
