...
Their separate owners are [`arm-pretool-check.md`](arm-pretool-check.md), [`cd-guard.md`](cd-guard.md), [`destructive-guard.md`](destructive-guard.md), and [`subagent-guard.md`](subagent-guard.md).
...
- Cursor registers a `stop` hook in `.cursor/hooks.json` and delegates the whole turn boundary to `bin/fm-turnend-guard-cursor.sh`, the park described below.
  ...
  The guarded set is the `SessionStart` entry, the three `PreToolUse` Bash entries, and both `Stop` entries.
