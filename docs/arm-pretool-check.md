...
`bin/fm-arm-command-policy.mjs` is also the sole owner of firstmate's shell classification: it exports the tokenizer, command-position analysis, reserved-word handling, and the directory-changing builtin set, which the sibling cd-guard seatbelt (`bin/fm-cd-pretool-check.sh`, `docs/cd-guard.md`) and destructive-command seatbelt (`bin/fm-destructive-pretool-check.sh`, `docs/destructive-guard.md`) reuse instead of duplicating shell lexing.
