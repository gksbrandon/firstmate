...
`bin/fm-arm-command-policy.mjs` is also the sole owner of firstmate's shell classification: it exports the tokenizer and command-position analysis, which the sibling cd-guard seatbelt (`bin/fm-cd-pretool-check.sh`, `docs/cd-guard.md`) and destructive-command seatbelt (`bin/fm-destructive-pretool-check.sh`, `docs/destructive-guard.md`) reuse instead of duplicating shell lexing.
